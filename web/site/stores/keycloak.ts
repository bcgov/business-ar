import Keycloak, { type KeycloakConfig, type KeycloakInitOptions, type KeycloakLoginOptions } from 'keycloak-js'
import { defineStore } from 'pinia'
import { SessionStorageKey } from '~/enums/session-storage'
import { type KCUser } from '~/interfaces/kc-user'
...
export const useBcrosKeycloak = defineStore('bcros/keycloak', () => {
  ...
  function clearSession () {
    ...
    sessionStorage.removeItem(SessionStorageKey.KEYCLOAK_TOKEN)
    sessionStorage.removeItem(SessionStorageKey.KEYCLOAK_TOKEN_REFRESH)
    sessionStorage.removeItem(SessionStorageKey.KEYCLOAK_TOKEN_ID)
    sessionStorage.removeItem(SessionStorageKey.KEYCLOAK_SYNCED)
  }

  function syncSessionStorage () {
    if (!kc.value) {
      sessionStorage.setItem(SessionStorageKey.KEYCLOAK_SYNCED, 'false')
      return
    }
    sessionStorage.setItem(SessionStorageKey.KEYCLOAK_TOKEN, kc.value?.token || '')
    sessionStorage.setItem(SessionStorageKey.KEYCLOAK_TOKEN_REFRESH, kc.value?.refreshToken || '')
    sessionStorage.setItem(SessionStorageKey.KEYCLOAK_TOKEN_ID, kc.value?.idToken || '')
    sessionStorage.setItem(SessionStorageKey.KEYCLOAK_SYNCED, 'true')
  }
