...
def upgrade():
    ...
    with op.batch_alter_table("business", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("corp_class", sa.String(length=10), nullable=True)
        )
        batch_op.create_index(
            batch_op.f("ix_business_corp_class"), ["corp_class"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_business_op_state"), ["op_state"], unique=False
        )
