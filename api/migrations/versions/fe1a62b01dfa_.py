...
def upgrade():
    ...
    with op.batch_alter_table("ar_reminders", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_ar_reminders_recipient"), ["recipient"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_ar_reminders_status"), ["status"], unique=False
        )
    ...
    with op.batch_alter_table("business", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column(
                "last_ar_reminder_date", sa.DateTime(timezone=True), nullable=True
            )
        )
...
def downgrade():
    ...
    with op.batch_alter_table("business", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "nano_id", sa.VARCHAR(length=25), autoincrement=False, nullable=True
            )
        )
